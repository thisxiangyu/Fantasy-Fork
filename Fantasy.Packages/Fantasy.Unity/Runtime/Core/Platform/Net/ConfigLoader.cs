...
public static class ConfigLoader
{
    ...
    /// <summary>
    /// 公开重载，供外部工具（如 DesignTime 工厂）调用。
    /// 默认不启用 Control Center 回退校验。
    /// </summary>
    public static List<WorldConfig> LoadWorldConfig(
        XmlNode serverNode,
        XmlNamespaceManager nsManager)
    {
        return LoadWorldConfig(serverNode, nsManager, false);
    }
        ...
        bool requireGroupId)
    {
        ...
        foreach (XmlNode worldNode in worldNodes)
        {
            ...
            var dbConfigs = new DatabaseConfig[databaseNodes.Count];
            var dbIndex = 0;
            ...
            foreach (XmlNode dbNode in databaseNodes)
            {
                ...
                dbConfigs[dbIndex++] = new DatabaseConfig(dbDuty, dbConnection, dbName, dbType, bool.Parse(isDefault));
            }
    ...
    private static DatabaseConfig GetDatabaseConfig(XmlNode dbNode)
    {
        ...
        var dbDuty = int.Parse(GetRequiredAttribute(dbNode, "duty"));
        var isDefault = GetOptionalAttribute(dbNode, "isDefault") ?? "false";
        ...
        return new DatabaseConfig(dbDuty, dbConnection, dbName, dbType, bool.Parse(isDefault));
    }
