...
namespace Fantasy.GlobalAndLocalization
{
    ...
    public enum 区域码 : uint
    {
        ...
        [共用区码(俄罗斯)]
        [语言码(nameof(语言映射.kk))]
        [语言码(nameof(语言映射.ru_kz))]
        [简写("KZ")]
        哈萨克斯坦 = 7,
        [大区(中亚)]
        [语言码(nameof(语言映射.ky))]
        [语言码(nameof(语言映射.ru_kg))]
        [简写("KG")]
        吉尔吉斯斯坦 = 996,
        [大区(中亚)]
        [语言码(nameof(语言映射.tg))]
        [语言码(nameof(语言映射.ru_tj))]
        [简写("TJ")]
        塔吉克斯坦 = 992,
        [大区(中亚)]
        [语言码(nameof(语言映射.tk))]
        [语言码(nameof(语言映射.ru_tm))]
        [简写("TM")]
        土库曼斯坦 = 993,
        [大区(中亚)]
        [语言码(nameof(语言映射.uz))]
        [语言码(nameof(语言映射.ru_uz))]
        [简写("UZ")]
        乌兹别克斯坦 = 998,
        ...
        [语言码(nameof(语言映射.es))] // 美国有极大量的西班牙语人口
        [简写("US")]
        美国 = 1,
        [大区(北美)]
        [共用区码(美国)]
        [语言码(nameof(语言映射.en_ca))]
        [语言码(nameof(语言映射.fr))] // 加拿大官方双语
        [简写("CA")]
        加拿大 = 1,
        ...
        [大区(拉丁美洲)]
        [语言码(nameof(语言映射.es))]
        [语言码(nameof(语言映射.gn))] // 瓜拉尼语 (巴拉圭官方双语)
        [简写("PY")]
        巴拉圭 = 595,
        [大区(拉丁美洲)]
        [语言码(nameof(语言映射.es))]
        ...
        [大区(欧洲)]
        [语言码(nameof(语言映射.es))]
        [语言码(nameof(语言映射.ca))] // 西班牙法律要求必须给予加泰语特别支持
        [简写("ES")]
        西班牙 = 34,
        [大区(欧洲)]
        [语言码(nameof(语言映射.pt))]
        ...
        [共用区码(意大利)]
        [语言码(nameof(语言映射.it))]
        [语言码(nameof(语言映射.la))] // 拉丁语 (属于古代语言了, 目前仅宗教性质使用)
        [简写("VA")]
        梵蒂冈 = 39,
        [大区(欧洲)]
        [语言码(nameof(语言映射.it))]
    ...
    public static class 区域码信息
    {
        ...
        static List<(区域码 大区, string 区码简写, 区域码 区码)> _region_area_number_tuples = null;
        ...
        /// </summary>
        public static List<(区域码 大区, string 区码简写, 区域码 区码)> 获取大区With简写With区码元组List(bool update_cache = false)
        {
            if (_region_area_number_tuples != null && !update_cache)
                return _region_area_number_tuples;

            _region_area_number_tuples = new List<(区域码 大区, string 简写, 区域码 区码)>();
            var type = typeof(区域码);

            foreach (var field in type.GetFields(BindingFlags.Public | BindingFlags.Static))
            {
                // 只处理带有语言码的元素（过滤掉纯大区定义元素）
                if (!field.IsDefined(typeof(语言码Attribute), false)) continue;

                var shortNameAttr = field.GetCustomAttribute<简写Attribute>();
                var regionAttr = field.GetCustomAttribute<大区Attribute>();
                var currentValue = (区域码)field.GetValue(null)!;
                ...
                _region_area_number_tuples.Add((regionAttr.大区, shortNameAttr.简写, currentValue));
            }
            return _region_area_number_tuples;
        }
    }
